...
export const TestimonialCard: React.FC<TestimonialCardProps> = ({ testimonial, date = "Jan 2026" }) => {
  ...
  const CardContent = ({ expanded = false }) => (
    <motion.div
      ...
      className={`
        flex flex-col bg-white dark:bg-accent-600/20 border border-neutral-100 dark:border-neutral-800 clip
        ${expanded 
          ? 'w-full max-w-2xl rounded-2xl p-8 shadow-2xl relative z-[1001] pointer-events-auto' 
          : 'w-72 rounded-xl px-4 pt-6 pb-4 shadow-sm hover:shadow-xl cursor-pointer relative z-10'
        }
      `}
    ...
    >
      {expanded && (
        <button
          ...
          className="absolute right-4 top-4 text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-200 transition-colors z-[1002]"
        >
      ...
      <motion.div className="flex gap-3 mb-2" layout="position">
        <motion.img
        ...
        <div className="flex flex-col justify-center">
          <motion.div className="flex items-center gap-1" layout="position">
            <p className={`font-semibold text-slate-900 dark:text-white leading-tight ${expanded ? 'text-xl' : 'text-base'}`}>
              {testimonial.name}
            </p>
            <svg className="mt-0.5 text-blue-500" width="12" height="12" viewBox="0 0 12 12" fill="none">
          ...
          <motion.span layout="position" className="text-xs text-slate-500 dark:text-slate-400 leading-tight">{testimonial.title}</motion.span>
        </div>
      </motion.div>

      <div className={` mb-4 custom-scrollbar ${expanded ? 'max-h-[60vh] pr-2 overflow-y-auto' : ''}`}>
        <motion.p 
          layout 
          className={`
            text-gray-700 dark:text-gray-300 pt-2
            ${expanded ? 'text-lg leading-relaxed whitespace-pre-line' : 'text-sm line-clamp-3'}
          `}
        >
